var gridsterTool = {
	...
	createGridster: function() {
		...
		gridsterTool.gridster = $(".gridster ul").gridster({
			...
			resize : {
				...
				stop : function(e, ui, $widget) {
					...
					semantic.flushChart(chart_id);
					gridsterTool.updatePosition();
				}
			}, // 模块右下角拖动大小默认为false
			draggable : {
		...
		$("#gridter_ul").find("li").each(function(){
			var id = $(this).attr("id");
			semantic.flushChart(id);
		});
	},
	...
	updatePosition: function() {
		...
		var dashboard_id = storageService.getParameter("semantic_dashboard_id");
		$.ajax({
			type: "post",
